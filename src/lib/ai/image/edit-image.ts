...
export async function removeImageBackground(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      try {
        const response = await fetch(`https://sii3.top/api/remove-bg.php?${params.toString()}`, {
          method: "GET",
        ...
        logger.info(`Remove BG: Background removed, downloading from ${imageUrl}...`);
...
export async function editImageWithNanoBanana(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      logger.info(`Nano-Banana Edit: Sending request with timeout of 60 seconds...`);
      ...
      try {
        ...
        logger.info(`Nano-Banana Edit: Image edited successfully, downloading from ${data.url}...`);
      ...
      lastError = error instanceof Error ? error : new Error(String(error));
      logger.warn(`Nano-Banana Edit attempt ${attempt} failed: ${lastError.message}`);
