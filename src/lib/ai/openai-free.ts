...
export function createOpenAIFreeModels() {
  ...
  const customFetch = async (
    ...
    try {
      ...
      const response = await fetch("https://sii3.top/api/openai.php", {
        method: "POST",
        headers: {
  ...
  const provider = createOpenAICompatible({
    name: "openai-free",
    apiKey: "free", // OpenAI doesn't require authentication
    baseURL: "https://sii3.top/api", // Dummy URL, we use custom fetch
    fetch: customFetch,
  });

  // Create model instances for OpenAI models
...
export const openAIFreeProvider = {
  models: createOpenAIFreeModels(),
  unsupportedToolModels: new Set<LanguageModel>([
    // OpenAI models don't support tool calls
  ]),
};
