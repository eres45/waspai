...
function BlueprintCanvas() {
  ...
  const models = ["Claude 3.5", "GPT-4o", "Gemini 1.5"];
  ...
  const isPathActive = (from: string, to: string) => {
    if (from === "trigger" && to === "model") return cycleStep === 1;
    if (from === "trigger" && to === "tool") return cycleStep === 2;
    if (from === "model" && to === "output") return cycleStep === 3;
    if (from === "tool" && to === "output") return cycleStep === 3;
    return false;
  };

  const getNodeStyle = (nodeId: string) => {
    let state: "active" | "completed" | "idle" = "idle";

    if (nodeId === "trigger") {
      if (cycleStep === 0) state = "active";
      else if (cycleStep >= 1 && cycleStep <= 4) state = "completed";
    } else if (nodeId === "model") {
      if (cycleStep === 1) state = "active";
      else if (cycleStep >= 2 && cycleStep <= 4) state = "completed";
    } else if (nodeId === "tool") {
      if (cycleStep === 2) state = "active";
      else if (cycleStep >= 3 && cycleStep <= 4) state = "completed";
    } else if (nodeId === "output") {
      if (cycleStep === 3) state = "active";
      else if (cycleStep === 4) state = "completed";
    }
  ...
  return (
    <div className="w-full bg-[#121214]/60 border border-white/5 rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
      <style
      ...
      {/* Visual Canvas */}
      <div className="flex flex-col justify-center bg-[#09090b]/40 rounded-xl border border-white/5 p-4 min-h-[300px] md:min-h-[350px] relative overflow-hidden select-none">
        <svg
        ...
        >
          ...
          <foreignObject x={210} y={20} width={130} height={80}>
            ...
            >
              ...
              <div className="text-left mt-1.5">
                <div className="flex items-center gap-1 text-[10px] font-bold text-white bg-white/5 px-1.5 py-0.5 rounded border border-white/5 leading-none w-fit">
                  {selectedModel}
                </div>
